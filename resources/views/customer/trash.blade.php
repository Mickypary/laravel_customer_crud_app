                            ...
                            <form action="{{ route('customers.index') }}">
                                <div class="input-group mb-3">
                                    <input type="text" name="search" value="{{ request()->search }}"
                                        class="form-control" placeholder="Search anything..."
                            ...
                            <form action="{{ route('customers.index') }}" class="form-order">
                                <div class="input-group mb-3">
                                    <select class="form-select" name="order" id=""
                                        ...
                                        <option {{ isset($_GET['order']) == 'desc' ? 'selected' : '' }} value="desc">
                                            Newest to
                                            Oldest</option>
                                        <option @selected(request()->order == 'asc') value="asc">Oldest to
                                            Newest</option>
                                    </select>
                                </div>
                            </form>
                            ...
                            @foreach ($customers as $key => $customer)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="javascript:;"
                                            onclick="if(confirm('Are you sure you want to delete this item?')) submit(<?php echo $customer->id; ?>)"
                                            style="color: #2c2c2c;" class="ms-1 me-1"><i class="fas fa-trash-alt"></i></a>
                                        {{-- document.getElementById("myForm").submit(); --}}
                                        {{-- $("#myForm").submit();  --}}
                                        {{-- $('.form-{{ $customer->id }}').submit() --}}
                                        <form class="form-{{ $customer->id }}"
                                            action="{{ route('customers.force.delete', $customer->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
...
@push('submit_script')
    <script>
        ...
        function submit(cus) {
            // alert(cus);
            document.querySelector(".form-" + cus).submit();
        }
    </script>
@endpush
